...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        """
        tile = self.pos_to_tile(pos)
        return tile in self.tiles and not self.is_position_furnished(pos)
    ...
    def get_random_position(self):
        ...
        while True:
            x = random.randint(0, self.width - 1)
            y = random.randint(0, self.height - 1)
            rand_pos = Position(x=x, y=y)
            if not self.is_position_furnished(rand_pos):
                break
        return rand_pos
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        next_pos = self.position.get_new_position(angle=self.direction, speed=self.speed)
        if self.room.is_position_valid(next_pos):
            self.position = next_pos
            self.room.clean_tile_at_position(pos=next_pos, capacity=self.capacity)
        else:
            self.direction = self.random_direction()
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        next_pos = self.position.get_new_position(angle=self.direction, speed=self.speed)
        if self.gets_faulty() or not self.room.is_position_valid(next_pos):
            self.direction = self.random_direction()
        else:
            self.position = next_pos
            self.room.clean_tile_at_position(pos=next_pos, capacity=self.capacity)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for _ in range(num_trials):
        ...
        robots = [robot_type(room=room, speed=speed, capacity=capacity) for _ in range(num_robots)]
...
#       of a 20x20 room?
# In this instance, a standard robot always performs better thant a faulty robot.
# As more and more robots get added, the difference becomes smaller.
#
# 2) How does the performance of the two robot types compare when two of each
#       robot cleans 80% of rooms with dimensions
#       10x30, 20x15, 25x12, and 50x6?
# In this instance, a standard robot performs significantly better.
# The chart shows that the optimal aspect ratio of a room for cleaning is a cubical
# or near cubical (aspect ratio of 1). The higher the aspect ratio, the poorer
# the performance of both types of robot. However, the performance of faulty robots
# deteriorates much more quickly as the aspect ratio moves further away from 1.
#
